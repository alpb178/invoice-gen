 ...
 * slug —"dando-muela" instead of "dandomuela"— would split the same metric into
 * two buckets nobody would reconcile later.
 *
...
/**
 * `null` if the link does not go to a group site: an anchor, a mailto, another site.
 *
 * It is resolved WITHOUT a fallback base on purpose. With one, a relative link
 * —`/empleos`— would resolve against this very site's domain and count as a
 * click leaving towards ourselves.
 */
export function resolveGroupSite(href: string, ownHost?: string): string | null {
  ...
  // A link to this same site is navigation, not a click that leaves.
