  ...
  if (!teamId) {
    return (
      <div className="w-full px-4 md:px-10 lg:px-16 py-10 text-center">
        <p className="text-ink-700 mb-4">{t('noTeam')}</p>
  ...
  return (
    <div className="w-full px-4 md:px-10 lg:px-16 py-8">
      ...
      <div className="bg-paper border border-ink-200 rounded-2xl p-6 mb-6 shadow-card">
        <h2 className="text-sm font-semibold text-ink-700 uppercase tracking-wider font-mono-tight mb-4">{t('team')}</h2>
        <ClearableField
          label={t('teamName')}
          value={form.name}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-paper border border-ink-200 rounded-2xl p-6 shadow-card">
          <h2 className="text-sm font-semibold text-ink-700 uppercase tracking-wider font-mono-tight mb-4">{t('issuer')}</h2>
          <div className="space-y-3">
            ...
            <ClearableField
              ...
              disabled={!isOwner}
              placeholder={te('addressPlaceholder')}
              multiline
              heightClass="h-20"
              inputClassName={inputClass}
              labelClassName={labelClass}
        ...
        <div className="bg-paper border border-ink-200 rounded-2xl p-6 shadow-card">
          <h2 className="text-sm font-semibold text-ink-700 uppercase tracking-wider font-mono-tight mb-4">{t('defaultClient')}</h2>
          <div className="space-y-3">
            <ClearableField
              label={te('name')}
      ...
      <div className="bg-paper border border-ink-200 rounded-2xl p-6 mb-6 shadow-card">
        <h2 className="text-sm font-semibold text-ink-700 uppercase tracking-wider font-mono-tight mb-4">{t('defaults')}</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className={labelClass}>{te('currency')}</label>
            <select
              disabled={!isOwner}
      ...
      {isOwner && (
        <div className="flex justify-end">
          ...
          >
            {saving ? te('saving') : t('saveChanges')}
          </button>
        </div>
      )}
    </div>
