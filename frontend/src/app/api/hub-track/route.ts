...
/**
 * Collects this site's analytics and forwards it to the group hub.
 *
 ...
 * session window, and it does not survive the browser being closed.
 *
 * It carries a random identifier and nothing else: no identity, no history. It
 * is the only thing that stops five pages from counting as five visits.
 */
 ...
 * The filter is deliberately coarse: what it lets through is a rounding error,
 * and what it would over-block would be a reader.
 */
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  // analytics problem must never look like a broken site, and telling the
  // caller why their payload was dropped only helps whoever is probing it.
  ...
  // The identifier is issued here and not in the page: as an httpOnly cookie no
  // script can read it, so a flaw in a third-party snippet can neither steal it
  // nor forge someone else's visit.
  ...
  noContent.cookies.set(SESSION_COOKIE, sessionId, {
    ...
    path: '/',
    // Renewed with every event, so the visit ends after half an hour of silence
  ...
  try {
    ...
    // The hub being down is not the visitor's problem. The event is dropped on
